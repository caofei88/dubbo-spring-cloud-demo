...
import com.springcloud.dubbo_provider.common.DubboResponse;
...
/**
    ...
    @Pointcut(value = "execution(* com.springcloud.dubbo_provider.service.*.*(..))")
...
/**
    ...
    */
/**
     * 异常处理切面 将异常包装为Response，避免dubbo进行包装 * * @param pjp 处理点 * @return Object
     *//*
    ...
    */
/**
     * 任何持有@Transactional注解的方法异常处理切面 将自定义的业务异常转为RuntimeException: * 1.规避dubbo的包装，让customer可以正常获取message 2.抛出RuntimeException使事务可以正确回滚 其他异常不处理 * * @param pjp 处理点 * @return Object
     *//*
    ...
    public Object doTransactionalAround(ProceedingJoinPoint pjp) throws Throwable {
        try {
            ...
            processException(pjp, args, e);
            // log.error("service with @Transactional exception occured on dubbo service
            // side: " + e.getMessage(), e);
